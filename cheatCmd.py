...
class cheatOptions(object):
    ...
    def checkOwner(self,clientID): 
        ...
        if client in MID.owners or MID.allAdmins: 
            bs.screenMessage('指令使用成功',color=(random.random(),random.random(),random.random()), clients=[clientID], transient=True)
            return True
    ...
    def set_tag(self,clientID):
        for i in bsInternal._getForegroundHostActivity().players:
            ...
            if i.getInputDevice().getClientID() == clientID:
                self.tag_client = i.get_account_id()
        print "cnm"
        print self.tag_client
  ...
  #defend your self from dhokebaaz admins......  
                    ...
                    if os.path.exists(tagfile):
                        ...
                        tag = {}
                    if a == []:
                        bs.screenMessage('没检测到你输入的标签', color=(1, 0, 0), clients=[clientID],
                                         transient=True)
                        tag[self.tag_client]=None
                        ...
                        for pattern in patterns:
                            if re.search(pattern, a[0]):
                                print "匹配成功: {}".format(pattern)
                                bs.screenMessage('好小子想造反啊', color=(1, 0, 0), clients=[clientID],
                                                 transient=True)
                                return


                        tag[self.tag_client]=a[0][0:12]
                        with open(tagfile, 'w') as f:
                            f.write(json.dumps(tag))
                        bs.screenMessage('标签保存成功: '+a[0][0:12], color=(0, 1, 0), clients=[clientID],
                                         transient=True)
                elif m == '/help':
                    _help_msg(clientID)
